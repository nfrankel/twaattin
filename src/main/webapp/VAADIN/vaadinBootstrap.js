(function() {
	...
	if (typeof console === "undefined"
		...
		// In IE, its a native function for which apply is not defined, but it
		// works without a proper 'this' reference
		log = console.log;
	}
...
window.vaadin = window.vaadin || {
				initApplication : function(appId, config) {
					...
					if (apps[appId]) {
						if (window.vaadin
								&& window.vaadin.clients
								&& window.vaadin.clients[testbenchId]
								&& window.vaadin.clients[testbenchId].initializing) {
					...
					var fetchRootConfig = function() {
						...
						if (!url) {
							// No special url defined, use the same URL that
							// loaded this page (without the fragment)
						...
						r.onreadystatechange = function(aEvt) {
							if (r.readyState == 4) {
								...
								if (r.status == 200) {
									...
									// Try bootstrapping again, this time
									// without fetching missing info
									bootstrapApp(false);
								} else {
					...
					if (getConfig("debug")) {
						// TODO debug state is now global for the entire page,
						// but should somehow only be set for the current
						// application
				...
				getBrowserDetailsParameters : function(parentElementId) {
					...
					for ( var m = 12; m > 0; m--) {
						...
						if (tzo1 != tzo2) {
							dstDiff = (tzo1 > tzo2 ? tzo1 - tzo2 : tzo2 - tzo1); // offset
																					// w/o
																					// DST
							rtzo = (tzo1 > tzo2 ? tzo1 : tzo2); // offset w/o
																// DST
							break;
						}
					}
